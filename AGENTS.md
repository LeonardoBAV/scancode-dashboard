...
- `filament-tenancy` — Use when implementing or changing Filament multi-tenancy (tenant switcher, `HasTenants`, `canAccessTenant`, tenant registration/profile, tenant middleware, `scopedUnique`/`scopedExists`, or scoping resources to a tenant). Follow `.cursor/skills/filament-tenancy/SKILL.md` and the official Filament 5 [Multi-tenancy](https://filamentphp.com/docs/5.x/users/tenancy) documentation. Do not use for generic Laravel-only scoping with no Filament panel tenancy.
